...
- ✅ 角色化对话（学生/教师）
- ✅ 管理员可配置/启用/默认大模型API
- ✅ 基础知识库（学科/文档）与教师个性化知识库并行
- ✅ 学生支持按课程选择“专属课程AI”并收藏
- ✅ 检索链路：教师KB优先 → 基础KB → 大模型生成（轻量RAG实现）

### 15. 向量检索方案建议（RAG演进）

当前项目已具备“知识库 + 流式问答”闭环：
- 基础知识库（管理员上传）与教师个性化知识库（教师上传）
- 生成链路中把检索到的文本片段拼接到提示词上下文，再调用大模型

该方案实现成本低、可运行，但当文档规模增大或需要更强语义检索时，建议按阶段演进：

**阶段 A：关键词检索（低成本，立刻可落地）**
- 目标：解决“资料能搜到/可解释”的基础需求。
- 推荐：SQLite FTS5（全文索引）或轻量 BM25 实现。
- 优点：依赖少、速度快、易调试；缺点：对同义词/语义相关召回弱。

**阶段 B：混合检索（兼顾精度与可控性）**
- 目标：在关键词召回基础上，引入更稳的排序策略。
- 方案：FTS5/BM25 召回 Top-K → 规则过滤（课程/学科/教师范围）→ 片段抽取（chunk）→ 送入大模型。
- 适用：文档中等规模、需要较稳定的可解释性。

**阶段 C：语义向量检索（Embedding + 向量库）**
- 目标：提高“语义相似”召回（同义表达、长问题、概念性提问）。
- 方案：Embedding（如 bge-m3 / text-embedding-*）→ 向量库（FAISS/Chroma/pgvector）→ 与关键词检索做融合（Hybrid RAG）。
- 注意：需要设计 chunk 切分、向量更新、权限过滤与成本控制；也要处理“幻觉”与引用追踪。

**何时必须上向量？（经验判断，不是硬阈值）**
- 关键词检索经常召回不到正确资料，且问题存在大量同义表达。
- 文档量增大后，Top-K 关键词结果噪声显著增多。
- 需要跨学科/跨课程的概念检索或总结类问题（语义覆盖更重要）。

**建议落地顺序**
1) 先用 SQLite FTS5/BM25 把“可检索+可解释”做稳；
2) 再做 chunk 结构化与权限过滤（按课程/教师/学科）；
3) 最后引入 embedding + 向量库，并做混合检索与回归评测。
