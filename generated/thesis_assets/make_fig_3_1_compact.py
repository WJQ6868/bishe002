...
font_body = ImageFont.truetype(r'C:\Windows\Fonts\simhei.ttf', 23)
...
d.text(((W - (bbox[2] - bbox[0])) / 2, 30), main_title, fill='black', font=font_title)
...
for i in range(4):
    ...
    d.text((lx, y - 42), lab, fill='black', font=font_small)

note = '说明：这张图把角色入口、前端页面、接口分组、AI能力和底层数据放到同一条设计链上，目的是先把结构关系讲清楚，后面再展开知识组织和数据表设计。'
draw_wrapped_text(110, 865, note, font_small, W - 220, line_gap=6, align='left')
