...
font_title = ImageFont.truetype(r'C:\Windows\Fonts\simhei.ttf', 44)
font_col = ImageFont.truetype(r'C:\Windows\Fonts\simhei.ttf', 29)
font_body = ImageFont.truetype(r'C:\Windows\Fonts\simhei.ttf', 24)
font_small = ImageFont.truetype(r'C:\Windows\Fonts\simhei.ttf', 21)

main_title = 'AI模块架构、代码组织与接口关系图'
bbox = d.textbbox((0, 0), main_title, font=font_title)
d.text(((W - (bbox[2] - bbox[0])) / 2, 36), main_title, fill='black', font=font_title)
...
cols = [
    ...
    },
    {
        'title': '前端页面与代码组织',
        'lines': [
            'views/admin/AdminAIConfig.vue',
            'views/teacher/CourseAssistant.vue',
            'views/teacher/LessonPlan.vue',
            'components/StudentAIChat.vue',
            'components/TeacherAIChat.vue',
            'api/adminAi.ts、aiPortal.ts、ai.ts'
        ]
    },
    {
        'title': '后端接口层',
...
def draw_wrapped_text(x, y, text, font, max_w, line_gap=7, align='left'):
...
for i, col in enumerate(cols):
    ...
    d.text((x + (col_w - (title_bb[2] - title_bb[0])) / 2, box_top + 28), col['title'], fill='black', font=font_col)

    y = box_top + header_h + 26
    inner_margin = 18
    max_w = col_w - inner_margin * 2
    for item in col['lines']:
        y = draw_wrapped_text(x + inner_margin, y, '· ' + item, font_body, max_w, line_gap=6, align='left')
        y += 12
...
for i in range(4):
    ...
    y = box_top + box_h / 2
    d.line((x1 + 8, y, x2 - 18, y), fill='black', width=4)
    d.polygon([(x2 - 18, y), (x2 - 42, y - 12), (x2 - 42, y + 12)], fill='black')
    lab = arrow_labels[i]
    bb = d.textbbox((0, 0), lab, font=font_small)
    lx = (x1 + x2 - (bb[2] - bb[0])) / 2
    d.text((lx, y - 46), lab, fill='black', font=font_small)

note = '说明：这张图把角色入口、前端页面、接口分组、AI能力和底层数据放到同一条设计链上，目的是先把结构关系讲清楚，后面再展开知识组织和数据表设计。'
draw_wrapped_text(110, 1335, note, font_small, W - 220, line_gap=6, align='left')
